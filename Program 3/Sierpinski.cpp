...
void divide_tetra(point a,point b,point c,point d,int i) {
	...
	if (i>0) {
		for (int j=0;j<3;j++) {
			v1[j]=(a[j] + b[j]) / 2;			v2[j]=(a[j] + c[j]) / 2;
			v3[j]=(a[j] + d[j]) / 2;			v4[j]=(b[j] + c[j]) / 2;
			v5[j]=(b[j] + d[j]) / 2;			v6[j]=(c[j] + d[j]) / 2;

		}
		divide_tetra(a,v1,v2,v3,i - 1);		divide_tetra(v1,b,v4,v5,i - 1);
		divide_tetra(v2,v4,c,v6,i - 1);		divide_tetra(v3,v5,v6,d,i - 1);
...
{
	std::cout << "Enter the number of iterations: ";	std::cin >> M;
	glutInit(&argv,argc);
